class @AgentEditPage
  constructor: ->
    ...
    $('form.agent-form').submit (e) ->
      ...
      if $(".link-region") && $(".link-region").data("can-receive-events") == false
        $(".link-region .select2-linked-tags option:selected").removeAttr('selected')

      if $(".control-link-region") && $(".control-link-region").data("can-control-other-agents") == false
        $(".control-link-region .select2-linked-tags option:selected").removeAttr('selected')

      if $(".event-related-region") && $(".event-related-region").data("can-create-events") == false

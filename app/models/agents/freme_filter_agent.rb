module Agents
  class FremeFilterAgent < Agent
    ...
    description <<-MD
      The `FremeFilterAgent` allows to execute a certain filter against a RDF graph sent as post body or as value of the NIF input parameter. For more information and a list of available filters, see the [Simply FREME output using SPARQL filters](http://api.freme-project.eu/doc/0.6/knowledge-base/filtering.html) article.
